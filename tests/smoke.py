...
def run(config_dir, log_path):
    ...
    try:
        ...
        def shell():
            ...
            if 'id="btn-theme"' not in text:
                raise AssertionError("shell is missing the interface markup")
        ...
        def prefs_on_disk():
            where = os.path.join(config_dir, "prefs.json")
            if not os.path.isfile(where):
                raise AssertionError("no prefs.json written to %s" % config_dir)
            with open(where, encoding="utf-8") as fh:
                stored = json.load(fh)
            if stored.get(EXAMINER, {}).get("theme") != "midnight":
                raise AssertionError("prefs.json lacks the stored theme: %r"
                                     % stored)
            return "prefs.json holds it"
        ...
        check("prefs reach prefs.json on disk", prefs_on_disk)
